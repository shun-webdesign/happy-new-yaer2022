...
const fragmentShader = `
  ...
  void main() {
    ...
    vec2 uv = fract(vUv * repeat - vec2(time, 1.)); // The sign of time change direction of movement

    vec3 texture = texture2D(uTexture, uv).rgb;
    // texture *= vec3(uv.x, uv.y, 2.); // To help visualize the repeated uvs

    gl_FragColor = vec4(texture, 2.);
  }
`;
